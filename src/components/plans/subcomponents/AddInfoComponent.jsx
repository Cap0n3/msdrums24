...
const InfoText = ({ text }) => (
    <Typography variant="caption" component="p" sx={{ mt: 1 }} fontWeight="bold">
...
const AdditionalInfoComponent = ({ additionalInfos }) => {
    return (
        <Box sx={{ width: '100%', mt: 4 }}>
            ...
            {Object.entries(additionalInfos.more_info).map(([key, values], index) => (
                <Box key={index} sx={{ mt: 1 }}>
                    <InfoText text={key} />
                    {values.map((value, idx) => (
                        <Typography variant="caption" component="p" key={idx} sx={{ ml: 2, mt: 0.5 }}>
                            - {value}
                        </Typography>
                    ))}
                </Box>
